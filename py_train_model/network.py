...
class Network(nn.Module):
    ...
    def load(self, modelpath,map_location=None):
        state = torch.load(modelpath,map_location)
        self.load_state_dict(state['policy']['net'])
...
class HexNetwork(Network):
    ...
    def forward(self,input):
        ...
        x = x.transpose(1, 2).contiguous() \
             .view(int(x.size(0)), 4, board_size, board_size)
        # resnet
