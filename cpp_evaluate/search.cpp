...
int SearchTree::alphabeta(node n, int depth, int alpha, int beta, bool player, int color)
{
	...
	if (depth == 0)
	{
		...
		struct parallelStruct
		{
			...
			bool playerCopy;
			int colorCopy;
			Evaluate::feature child_posCopy;
		};
		
		parallelStruct* pParallel = new parallelStruct[t];

		for (int index = 0; index < t; ++index)
		{
			/* code */
			Copy_node((pParallel[index].childCopy),n);
			pParallel[index].child_posCopy = children_pos[index + 1];
			int i = pParallel[index].child_posCopy.p / SIZE;
			int j = pParallel[index].child_posCopy.p % SIZE;
			pParallel[index].alphaCopy = alpha;
			pParallel[index].betaCopy = beta;
			pParallel[index].playerCopy = HUMAN;
			pParallel[index].colorCopy = color;
			Move(color,pos(i,j),pParallel[index].childCopy);
			Unmove(pos(i,j));
		}
		...
		#pragma omp parallel for
			for (int i = 0; i < t; i++)
			{
				/* code */
				omp_set_lock(&lock);
				int v = alphabeta(pParallel[i].childCopy, 1, pParallel[i].alphaCopy, pParallel[i].betaCopy, pParallel[i].playerCopy, pParallel[i].colorCopy);
				
				this->Move_Array[pParallel[i].child_posCopy.p] = v ; //È¡ÕýÊý
				omp_unset_lock(&lock);
			}
			/*
			for( int index = 1; index <= t;index ++)
			{
				cout<<"parallel: "<<index<<endl;
				int v;
				node child;
				...
				omp_set_lock(&lock);
				this->Move_Array[child_pos.p] = v ; //È¡ÕýÊý
				omp_unset_lock(&lock);
				Unmove(pos(i, j));
			}
			*/
